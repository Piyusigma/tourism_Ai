...
export default function ExperienceScreen({ result, imageUrl, onBack }) {
  ...
  return (
    <motion.div
      className="min-h-[100dvh] bg-transparent text-[var(--text-1)] overflow-x-hidden pb-32"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      ...
      <div className="fixed top-0 left-0 right-0 z-50 pt-6 pointer-events-none">
        <div className="w-full px-6 py-3 flex items-center justify-between pointer-events-auto">
          <motion.button
      ...
      <div className="w-full px-6 md:px-12 lg:px-24 mx-auto max-w-screen-2xl">
        ...
        <FadeIn className="mb-24 md:mb-32">
          <div className="w-full">
            ...
            <p
              className={`text-[var(--text-secondary)] text-xl md:text-3xl lg:text-4xl leading-[1.6] md:leading-[1.7] tracking-tight whitespace-pre-line font-light w-full ${isTyping ? 'typewriter-cursor' : ''
                }`}
            >
              {displayedText}
            </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full">
                {result.funFacts.map((fact, i) => (
                  <FadeIn key={i} delay={i * 0.1}>
                    <div className="glass-card p-8 h-full flex flex-col justify-center items-center text-center group hover:border-[var(--border-medium)] transition-all duration-500 hover:bg-white/5">
